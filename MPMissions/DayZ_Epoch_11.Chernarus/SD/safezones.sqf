...
[
	...
	[[12060,12640,0],100,true],//air
	[[4361.4937,2259.9526,0],50,true],//wholesalerSouth
	[[13532.614,6355.9497,0],50,true],//wholesalerEast
	//[[6569,14216,0],50,true]//SHero
	
	
//BANKS
	[[10005.678,1861.6836,0],15,true],
	[[12214.157,9731.7646,0],15,true],
	[[13343.663,5488.4917,0],15,true],
	[[1008.7841,2218.9775,0],15,true],
	[[8666.5801,6517.2212,0],15,true],
	[[3649.0173,8532.2158,0],15,true]
];
	...
	{
		{
			...
			if (_lSign) then
			{
				for '_i' from 0 to 360 step (270 / _radius)*2 do
				{
					...
					_object = createVehicle ['Sign_Checkpoint_US_EP1', _location, [], 0, 'CAN_COLLIDE'];
...
[_infiSZ] spawn {
	...
	_startSafeZone =
	{
		...
		{
		["<t size='1.5' shadow ='0.6' color='#B22222'>SAFEZONE: GEAR AND VEHICLES CAN BE STOLEN, LOCK AND PROTECT YOUR STUFF!!!</t>",0,(safezoneY + safezoneH * 0.43),7,1,0,32] spawn bis_fnc_dynamicText;
...
private ["_cursorTarget","_inViewPlayer","_inViewOwnerID","_friendlies"];		
		while {1 == 1} do
		{
			if (inSafezone) then
			{
				if (_USE_AntiSteal) then
				{
					...
					_inViewPlayer = isPlayer _cursorTarget;
					_inViewOwnerID = _cursorTarget getVariable["CharacterID","0"];
					_friendlies	= player getVariable ["friendlyTo",[]];
					
					if ( _inViewPlayer ) then {
						if(_inViewOwnerID in _friendlies) then { 
							systemchat "You can trade with that player!";
						} else {		
