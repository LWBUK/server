...
"_allowed","_hasAccess","_uid","_isAnimal","_hasKeyKit","_isLocked"];
...
if (_isMan and !_isAlive and !_isZombie and !_clothesTaken) then {
    if (s_player_clothes < 0) then {
        s_player_clothes = player addAction [format["<t color='#0096ff'>Take Clothes</t>"], "SD\takeclothes\takeClothes.sqf",_cursorTarget,0, false,true];
    };
} else {
...
//	};
	...
	if (_typeOfCursorTarget in DZE_MoneyStorageClasses && {!locked _cursorTarget} && !(_typeOfCursorTarget in DZE_LockedStorage) && {player distance _cursorTarget < 5}) then {
		if (s_bank_dialog < 0) then {
			s_bank_dialog = player addAction [format ["<t color=""#0096FF"">Coin Storage</t>"], "scripts\zsc\bankDialog.sqf",_cursorTarget, 1, true, true];
		};
	} else {
	...
	if (_typeOfCursorTarget in DZE_MoneyStorageClasses && {!locked _cursorTarget} && !(_typeOfCursorTarget in DZE_LockedStorage) && {player distance _cursorTarget < 5}) then {
		if (s_bank_dialog1 < 0) then {
			s_bank_dialog1 = player addAction [format ["<t color=""#0096FF"">Online Banking</t>"], "scripts\zsc\atmDialog.sqf",_cursorTarget, 1, true, true];
		};
	} else {
		player removeAction s_bank_dialog1;
		s_bank_dialog1 = -1;
	...
	if (_isAlive && {_typeOfCursorTarget in AllPlayers} && {isPlayer _cursorTarget}) then {
		if (s_givemoney_dialog < 0) then {
			s_givemoney_dialog = player addAction [format ["<t color=""#0096FF"">Trade %1 with %2</t>",CurrencyName,name _cursorTarget], "scripts\zsc\givePlayer.sqf",_cursorTarget, 3, true, true];
		};
	} else {
