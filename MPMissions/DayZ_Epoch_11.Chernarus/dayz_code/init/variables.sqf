...
ZSC_MaxMoneyInStorageMultiplier = 10000; // Multiplier for how much money a bank object can hold, example: 200 magazine slots in the object (or the default value above ^^) multiplied by the 50,000 multiplier is 10 million coin storage. (200*50000=10m coins)

Bank_Object = ["WarfareBunkerSign"]; //add objects that you want to use as ATMs.
LimitOnBank = true; // false = no limits, true = banks are limited on the value below
MaxBankMoney = 20000000; // limit on bank for players 20mil
