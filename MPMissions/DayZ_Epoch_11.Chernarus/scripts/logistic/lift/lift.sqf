if (LOG_INPROGRESS) then {
	...
	_object = (position _heliporteur) nearEntities [["LandVehicle","Air","Ship"],20];
				...
				if (count crew _object == 0) then {	
					if (isNull (_object getVariable "LOG_moves_by") || (!alive (_object getVariable "LOG_moves_by"))) then {
						...
						if (_no_trailer) then {
							...
							format [STR_LOG_ATTACHED,getText (configFile >> "CfgVehicles" >> (typeOf _object) >> "displayName")] call dayz_rollingMessages;
						} else {
							format [STR_LOG_CANT_LIFT_TOWING,getText (configFile >> "CfgVehicles" >> (typeOf _object) >> "displayName")] call dayz_rollingMessages;};
					} else {
						format [STR_LOG_CANT_LIFT_MOVING,getText (configFile >> "CfgVehicles" >> (typeOf _object) >> "displayName")] call dayz_rollingMessages;};	
				} else { 
					format [STR_LOG_CANT_LIFT_PLAYER,getText (configFile >> "CfgVehicles" >> (typeOf _object) >> "displayName")] call dayz_rollingMessages;};
			} else { 
				format [STR_LOG_ALREADY, getText (configFile >> "CfgVehicles" >> (typeOf _object) >> "displayName")] call dayz_rollingMessages;};
		};
	};
